...
- **No sign-in, fully local:** built with the `skip_login` feature — Clinch opens straight to the terminal with no account or login/user UI at all, and authenticated calls to Warp's servers hard-fail by design, so it never phones home. Download and use it immediately.
- **Cloud features removed:** the backend-only surfaces — Warp AI / agent mode, Warp Drive, teams/billing, and session sharing — are hidden in this build (they can't work without Warp's servers), leaving a clean local terminal. Bring your own CLI agent (Claude Code, Codex) instead.
