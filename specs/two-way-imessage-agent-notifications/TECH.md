...
- Model the iMessage category as a presentation state machine derived from `IMessageConfiguration` and `IMessageConnectionStatus`: enter number, connecting, Full Disk Access, Automation, Messages sign-in, awaiting calibration reply, connected, and error. The destination input uses a labeled **Connect** submit action while retaining Return submission. A valid submission saves an enabled-by-default but not-yet-effective configuration, starts the bridge, and opens `Privacy_AllFiles` immediately. The permission state instructs the user to quit and reopen; coordinator initialization resumes setup from the saved number without a manual health refresh.
- Replace the editable destination field after submission with a number chip and state-specific action. The connected state exposes serialized **Test** delivery with ephemeral idle/sending/sent/failed coordinator state and **Change number**. Changing the number clears conversation correlation and queued phone content while preserving live durable-session routes, explicit notification overrides, and the notification default for the next connection. Show the global connection and session-notification-default switches only after calibration completes, both on by default.
...
- Settings tests cover the setup-stage derivation and instantiate the iMessage category alongside its required models. Footer presentation tests cover the immediate post-submit state, permission remediation, connected Yes/No preference, and globally paused delivery. Compile-time session integration and exact `CLIAgentSessionKey` revalidation cover the external PTY entry point; real Messages/TCC behavior remains part of the release matrix below.
