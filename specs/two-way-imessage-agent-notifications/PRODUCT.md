...
2. The feature is disabled until setup succeeds. Settings presents one guided setup state at a time: **Set up your phone**, a phone-number field, and an explicit **Connect** action (Return is the keyboard equivalent). Submitting a valid number persists it immediately, replaces the input with a pill containing that number, updates supported agent footers from **Set up iMessage** to the current setup state, and explains that messages use the Messages account already signed in on the Mac. It does not ask the user to install or configure an iPhone app.

3. Connect immediately opens the Full Disk Access pane in System Settings. The in-app state tells the user to enable Clinch, quit, and reopen it; setup then resumes automatically with the persisted number. Messages sign-in, macOS Automation access, and Full Disk Access are each shown as a specific actionable state instead of a generic failure or a manual Refresh button.
