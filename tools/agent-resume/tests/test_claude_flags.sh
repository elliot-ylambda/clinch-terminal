...
out="$(_clinch_agent_resume_extract_flags "node /x/claude-code/cli.js --dangerously-skip-permissions")"
...
out="$(_clinch_agent_resume_extract_flags "claude --permission-mode plan")"
...
out="$(_clinch_agent_resume_extract_flags "claude --permission-mode=acceptEdits")"
...
out="$(_clinch_agent_resume_extract_flags "claude --dangerously-skip-permissions --model opus")"
[[ "$out" == *"--dangerously-skip-permissions"* && "$out" == *"--model opus"* ]] || fail "model space form (got: '$out')"
out="$(_clinch_agent_resume_extract_flags "claude --model=sonnet")"
[[ "$out" == *"--model sonnet"* ]] || fail "model equals form (got: '$out')"

# 5. No relevant flags -> empty.
out="$(_clinch_agent_resume_extract_flags "node /x/claude-code/cli.js")"
[[ -z "$out" ]] || fail "expected empty for no flags (got: '$out')"

# 6. Unrelated flags (incl. a stale --resume) are ignored: we only carry mode + model.
out="$(_clinch_agent_resume_extract_flags "claude --verbose --resume old-id")"
[[ -z "$out" ]] || fail "unrelated flags must be ignored (got: '$out')"

# 7. _clinch_agent_resume_extract_model carries only --model, never the mode flags.
out="$(_clinch_agent_resume_extract_model "node /x/cli.js --dangerously-skip-permissions --model opus")"
[[ "$out" == " --model opus" ]] || fail "extract_model carries only the model (got: '$out')"
out="$(_clinch_agent_resume_extract_model "claude --model=sonnet")"
[[ "$out" == " --model sonnet" ]] || fail "extract_model equals form (got: '$out')"
out="$(_clinch_agent_resume_extract_model "claude --permission-mode plan")"
...
out="$(_clinch_agent_resume_mode_flags_from_payload "bypassPermissions")" || fail "bypassPermissions must map"
[[ "$out" == " --dangerously-skip-permissions" ]] || fail "bypassPermissions mapping (got: '$out')"
out="$(_clinch_agent_resume_mode_flags_from_payload "plan")" || fail "plan must map"
[[ "$out" == " --permission-mode plan" ]] || fail "plan mapping (got: '$out')"
out="$(_clinch_agent_resume_mode_flags_from_payload "acceptEdits")" || fail "acceptEdits must map"
[[ "$out" == " --permission-mode acceptEdits" ]] || fail "acceptEdits mapping (got: '$out')"
out="$(_clinch_agent_resume_mode_flags_from_payload "default")" || fail "default must map (to nothing)"
[[ -z "$out" ]] || fail "default maps to no flags (got: '$out')"
_clinch_agent_resume_mode_flags_from_payload "somethingNew" >/dev/null && fail "unknown mode must return nonzero"
_clinch_agent_resume_mode_flags_from_payload "" >/dev/null && fail "empty mode must return nonzero"
